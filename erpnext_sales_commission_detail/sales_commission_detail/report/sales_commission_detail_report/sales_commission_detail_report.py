...
def get_invoice_items(filters):
	...
	return frappe.db.sql("""
		...
		FROM
			...
			GROUP BY customer_name, name WITH ROLLUP LIMIT 100 ) AS t_inv
		LEFT JOIN `tabSales Invoice Item` AS t_invitem ON t_invitem.parent = t_inv.name
		WHERE t_inv.sales_partner = '{sales_partner}' AND t_inv.posting_date >= '{from_date}' AND t_inv.posting_date <= '{to_date}' """.format(
